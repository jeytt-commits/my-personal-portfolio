...
1. **Reaching 25 different elements:** Created a checklist of HTML5 elements and deliberately included less common ones (`details`, `summary`, `figure`, `figcaption`, `dl/dt/dd`, `address`, `time`, `mark`, `abbr`, `cite`, `blockquote`, `pre`, `code`, `fieldset`, `legend`, etc.).
2. **Making a pure-HTML site look organized:** Used clear headings, horizontal rules (`<hr>`), lists, and a table. Semantic structure helped a lot.
3. **Form accessibility:** Carefully paired every `<label>` with an `id` using the `for` attribute and added `required` + `placeholder`.
4. **Valid nesting of table elements:** Referenced MDN documentation for correct `<thead>`, `<tbody>`, `<tfoot>`, and `scope` usage.
5. **Keeping the code readable:** Consistent indentation, meaningful comments, and logical section ordering.
...
git commit -m "Initial commit: Create my personal portfolio with semantic HTML"
git branch -M main
...
- opened and Closed all tags correctly and maintained correct nesting.
...
7. Deploy the site using GitHub Pages for a live public URL.
